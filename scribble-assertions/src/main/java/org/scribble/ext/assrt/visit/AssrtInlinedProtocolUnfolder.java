...
public class AssrtInlinedProtocolUnfolder extends InlinedProtocolUnfolder
{
	...
	public void setRecVar(AstFactory af, RecVar recvar, Recursion<?> rec) throws ScribbleException
	{
		...
		else
		{
			...
			unfolded = lr.reconstruct(rv, (LProtocolBlock) block, lr.annotvars, lr.annotexprs,
					lr.ass);  // FIXME: factor out better? -- could just use clone and replace del? (no, need to set new block)
		}
