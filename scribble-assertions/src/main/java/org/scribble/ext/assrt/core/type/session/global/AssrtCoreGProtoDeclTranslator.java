...
{
	public static final DataName UNIT_DATATYPE = new DataName("_Unit");  // FIXME: move
	...
	/*
	// Mostly duplicated from parseGMsgTransfer, but GMsgTransfer/GConnect have no useful base class 
	private AssrtCoreGConnect parseGConnect(GConnect gc) throws F17Exception 
	{
		...
		if (!gc.msg.isSigNode())
		{
			throw new F17SyntaxException(gc.msg.getSource(), " [f17] Msg kind not supported: " + gc.msg);
		}
		SigNode msn = ((SigNode) gc.msg);
		Op op = msn.op.toName();
		Payload pay = null;
	...
	*/
			...
			boolean checkChoiceGuard, boolean checkRecGuard) throws F17Exception
	{
		...
		{
			if (first instanceof GMsgTransfer)
			{
				AssrtCoreGMsgTransfer gmt = parseGMsgTransfer((GMsgTransfer) first);
				AssrtCoreGType cont = parseSeq(jc, mc, is.subList(1, is.size()), false, false);
				Map<AssrtCoreGAction, AssrtCoreGType> cases = new HashMap<>();
					cases.put(gmt, cont);
				return this.factory.GChoice(cases);
		...
		else
		{
			...
			if (first instanceof GChoice)
			{
				...
				for (GProtoBlock b : gc.getBlocks())
				{
					...
					for (Entry<AssrtCoreGAction, AssrtCoreGType> e : tmp.cases.entrySet())
					{
						...
						if (k.isMsgAction())
						{
							if (cases.keySet().stream().anyMatch((x) ->
									x.isMsgAction() && ((F17MsgAction) k).getOp().equals(((F17MsgAction) x).getOp())))
							{
	...
	private AssrtCoreGMsgTransfer parseGMsgTransfer(GMsgTransfer gmt) throws F17Exception 
	{
		...
		if (!gmt.msg.isSigNode())
		{
			throw new F17SyntaxException(gmt.msg.getSource(), " [f17] Not supported: " + gmt.msg);  // TODO: SigName
		}
		SigNode msn = ((SigNode) gmt.msg);
		Op op = msn.op.toName();
		Payload pay = null;
		if (msn.payloads.getElements().isEmpty())
		{
		...
		return this.factory.GMsgTransfer(src, dest, op, pay);
	}
