...
public class ChoiceTypeBuilder implements IRustMpstBuilder {
	...
	public ChoiceTypeBuilder(ArrayList<IRustMpstBuilder> paths, BuilderKind kind, Role self, List<Role> otherRoles) {
		this.paths = paths;
		this.kind = kind;
		this.self = self;
	...
	// example:
	// type ChooseCtoB<N> = ChooseMpst<AtoBVideo<N>, CtoBClose, AtoBClose,
	// CtoBClose, QueueBVideoDual, QueueBEnd>;
	// as many ChooseTypes as the OtherRoles
	private String buildMpstChoice() {
		StringBuilder sb = new StringBuilder();
		StringBuilder qb = new StringBuilder();
		String simpleTypes = this.paths.stream().map(t -> t.build() + "\n").reduce("", String::concat);
		...
		String choiceQueue = String.format("type %s = Role%stoAll<%s, %s>;", this.execOrderName, this.self.toString(),
		...
		for (int i = 0; i < this.otherRoles.size(); i++) {
			...
			String name = String.format("%s%sto%s<N>", this.getKindString(), this.self.toString(), role);
			...
			List<String> binaryNames = this.paths.stream()
					.map(t -> ((RustMpstSessionBuilder) t).rolesToTypeNames.get(role)).collect(Collectors.toList());
