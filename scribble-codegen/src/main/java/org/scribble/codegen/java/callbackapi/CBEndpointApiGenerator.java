		...
		for (EState s : states)
		{
			...
			for (EAction a : s.getActions())
			{
				stateClass += "this.succs.put(" + SessionApiGenerator.getEndpointApiRootPackageName(this.proto) + ".ops." + SessionApiGenerator.getOpClassName(a.mid) + "." + SessionApiGenerator.getOpClassName(a.mid)
						...
						+ "\"" + ((s.getDetSucc(a).getStateKind() == EStateKind.TERMINAL) ? "End" : this.proto.getSimpleName() + "_" + this.self + "_" + s.getDetSucc(a).id) + "\""
						+ ");\n";
			}
			stateClass += "}\n";
			stateClass += "}\n";
			...
			if (s.getStateKind() == EStateKind.OUTPUT)
			{
				...
				for (EAction a : s.getActions())
				{
					...
					messageClass += "return "
							+ SessionApiGenerator.getEndpointApiRootPackageName(this.proto)
							+ ".ops." + SessionApiGenerator.getOpClassName(a.mid) + "."
			...
			if (s.getStateKind() == EStateKind.UNARY_RECEIVE || s.getStateKind() == EStateKind.POLY_RECIEVE)
			{
				...
				branchAbstract += s.getActions().stream()
						.map(a -> ", " + pack + ".handlers." + this.self + ".interfaces."
								+ name + "__" + a.peer + "_"
								+ SessionApiGenerator.getOpClassName(a.mid)
								+ a.payload.elems.stream().map(e -> "_" + e)
				...
				for (EAction a : s.getActions())
				{
					String receiveIfName = name + "__" + a.peer + "_"
							+ SessionApiGenerator.getOpClassName(a.mid) + a.payload.elems
									.stream().map(e -> "_" + e).collect(Collectors.joining());
					...
					branchAbstract += "case \""
							+ SessionApiGenerator.getOpClassName(a.mid) + "\": receive(data, "
							+ SessionApiGenerator.getEndpointApiRootPackageName(this.proto)
							+ ".roles." + a.peer + "." + a.peer + ", ";
					if (isSig)
					{
