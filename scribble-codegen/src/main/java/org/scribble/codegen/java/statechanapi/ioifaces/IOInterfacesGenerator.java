...
import org.scribble.codegen.java.statechanapi.CaseSockGen;
...
public class IOInterfacesGenerator extends ApiGen
{
	...
	private void generateIOStateInterfacesFirstPass(Set<EState> visited, EState s) throws ScribException
	{
		...
		{
			generateIOStateInterfacesFirstPass(visited, s.getDetSucc(a));
		}
	}
	...
	private void generateIOStateInterfacesSecondPass(Set<EState> visited, EState s)
	{
		...
		for (EAction a : s.getDetActions())
		{
			generateIOStateInterfacesSecondPass(visited, s.getDetSucc(a));
		}
	}
	...
	private void generateHandleInterfaces(Set<EState> visited, EState s) throws ScribException
	{
		...
		for (EAction a : s.getDetActions())
		{
			generateHandleInterfaces(visited, s.getDetSucc(a));
		}
	}
	...
	private void generateHandleInterfacesSecondPass(Set<EState> visited, EState s)
	{
		...
		for (EAction a : s.getDetActions())
		{
			generateHandleInterfacesSecondPass(visited, s.getDetSucc(a));
		}
	}
	...
	private void addIOStateInterfacesToStateChannels(Set<EState> visited, EState s)
	{
		...
		for (EAction a : s.getDetActions())
		{
			addIOStateInterfacesToStateChannels(visited, s.getDetSucc(a));
		}
	}
	...
	private String getConcreteSuccessorParameters(EState s)
	{
		return "<" +
				s.getDetActions().stream().sorted(IOStateIfaceGen.IOACTION_COMPARATOR)
						.map((a) -> this.getSuccName.apply(s.getDetSucc(a))).collect(Collectors.joining(", "))
				+ ">";
	}
	...
	private void collectBranchSuccs()
	{
		...
		for (EState s : this.branchPostActions.keySet())
		{
			...
			this.branchPostActions.get(s).forEach((a) -> curr1.addAll(s.getDetSucc(a).getDetActions()));  // TODO: flatmap
